...
- ⚠️ **`db.sync_pending_bets_from_github()` / `run_github_bet_sync_loop()`
  (the Railway `--bot` thread that feeds Railway's settlement/CLV loops
  with picks GH Actions just generated) is now broken.** It works by
  fetching `parlay_os.db` from
  `raw.githubusercontent.com/ahow1/parlay-os/main/parlay_os.db` — a URL
  that 404s now that the file isn't in the tree. Until this is replaced,
  **Railway will not learn about any new pick generated by GitHub Actions
  after 2026-07-28** (existing pending bets already synced before this
  change are unaffected). Needs a replacement sync path that doesn't
  require committing the DB publicly — e.g. GH Actions POSTs new pending
  bets to a Railway API endpoint directly over HTTPS instead of Railway
  pulling a public file. Not yet built.
