...
| `live_engine.py` | In-game live-betting alerts — `run_live_pass()` (one poll per GH Actions cron tick, only for games with an open position; see Known Bugs) |
| `narrative_engine.py` | One Claude Haiku API call per slip — writes the real plain-English "Why" narrative for every displayed pick, grounded to the model's own structured factor data; template narrative (`brain._pick_narrative`) is the fallback on any failure |
| `odds_quota.py` | The Odds API monthly-allowance tracking + hard guard that disables `live_engine` (never scouts) past 80% usage |
| `odds_quota_state.json` | Cross-run persistence for `odds_quota.py`'s "disabled for the rest of the month" flag — committed to git like `last_scout.json`, since GH Actions jobs don't share a local DB |
...
- Live engine: cut down 2026-09-09 to a single pass per cron tick (every 15 min, was 5), polling
  ...
  disables it (never scouts) for the rest of the month — see Known Bugs. Staying on The Odds API's
  free tier (500 credits/month) for now; this guard is load-bearing until/unless that changes.
...
| Telegram slip unreadable on busy days — 100+ full pick blocks | `_render_slip_picks()` rendered one full detail block (matchup, odds, stake, model%/market%/edge%, Why narrative, key drivers, fallback flags) per qualifying pick, staked AND over-cap alike, with no per-category limit — a busy slate's over-cap player props (uncapped by count, only cut by the props $ pool budget) alone could push 100+ blocks into one slip. Redesigned 2026-09-09, revised 2026-09-10, into 5 labeled sections, every capped section ranking staked+over-cap combined by edge% ("best picks it saw," not "first picks it found"): SECTION 1 — ML/F5 (unlimited, usually 2-5/day), SECTION 2 — top 5 TOTAL picks by edge%, SECTION 3 — NRFI/YRFI top 5 by edge% (fewer if fewer qualify), SECTION 4 — top 3 RUNLINE picks by edge%, SECTION 5 — top 5 player props by edge% (hitter+pitcher combined: Ks/ER/TB/HR/RBI/Hits/SO). Only PARLAY/SGP has no section at all and is always fully suppressed; TOTAL/NRFI got their own sections in the 2026-09-10 revision after initially being suppressed entirely in the first pass. Everything not shown is unaffected on the DB-logging side — `_log_pick_with_retry()` calls upstream in `_daily_bet_slip` are untouched — one footer line reports what was suppressed, e.g. `📊 +87 props, +2 runlines, +3 NRFI, +1 parlays, +4 totals logged for CLV tracking (not shown)`. Also fixed in the same pass: standalone prop plays' `event` field fell back to a bare team code (e.g. "⚾ TOR") instead of the full matchup ML/TOTAL plays always show — `_prop_slip_pick()` now uses the real `game` field (always populated by the hitter/K/ER-prop pipelines) directly, with a labeled `"TOR (matchup unavailable)"` fallback instead of a bare code for the hypothetical case it's ever missing. Display-layer only — no change to database logging, CLV capture, or diagnostic storage. (`test_transparency_slip.py::TestSlipSectionRedesign`, plus updated assertions in `test_runline.py`/`test_wire_ins.py` that previously expected NRFI/TOTAL to render inline) |
| Slip "Why" narrative read like a log line, not a bettor's reasoning | `_pick_narrative()`'s template text (`"X's edge over Y is led by key reliever availability (their key reliever unavailable (+1.2pp for us, 1 flagged))..."`) was accurate but mechanical — raw factor labels/values concatenated, not something a person would actually say. Fixed 2026-09-10 by adding `narrative_engine.py`: exactly ONE Claude Haiku (`claude-haiku-4-5-20251001`) API call per SLIP (not per pick) — passes every displayed pick's already-distilled factor breakdown (the same `drivers`/`neutral_fallbacks` `_pick_narrative()` computes, never raw internal diagnostics) plus odds/model/market context, gets back a 2-4 sentence narrative per pick (lead driver in plain terms → market-vs-model gap in odds terms, e.g. "market has KC at +172, the model sees closer to +130" → one sentence on what would change the call) and a humanized driver-line list (e.g. `"Bullpen: MIN late innings fatigued (+1.2pp)"`). The system prompt's hard grounding rule forbids introducing any fact not in the given data and forbids hype language ("lock", "smash", etc.); the deterministic template stays as both the grounding-data source and the automatic per-pick fallback — a missing index, malformed JSON, timeout, missing API key, or any other failure degrades that pick's block to the old template text, never blocks or delays the slip send. `REQUEST_TIMEOUT_SEC=20` on the API call keeps a worst-case hang from eating into the scout's GH-Actions time budget. Logs input/output tokens + an estimated cost per call; a realistic 13-pick slip estimates to roughly a cent (character-count-based estimate — no live API access was available to measure real tokenization when this was built). Requires `ANTHROPIC_API_KEY` as a **GitHub Actions** secret on the 3 scout jobs (previously only needed on Railway) and `anthropic` added to those jobs' `pip install` line — see Required Secrets. (`test_narrative_engine.py`, `test_transparency_slip.py::TestNarrativeEngineMerge`) |
| `live_engine` was the largest odds-quota consumer, exhausting the free tier mid-month | `brain.py --live` called `run_live_monitor()`, a continuous 60-second-cycle loop that GH Actions' `timeout 5m` step killed after ~5 cycles — so every 5-minute cron tick (two windows, ~60 ticks/night) actually made up to 5 rounds of real Odds API calls, not 1, since none of the CI jobs installs `requests_cache` (compare `requirements.txt` to the workflow's `pip install` line) so the usual 5-minute HTTP cache never applied in production. On top of that, every poll scanned the *entire* live MLB slate (10-15 games) for brand-new comeback opportunities regardless of whether we held any position. Combined, this is what exhausted the free 500-credit/month tier mid-month and caused the silent multi-day odds-feed outages for the daily scouts. Fixed 2026-09-09: (1) `brain.py --live` now calls the new `live_engine.run_live_pass()` — a true single pass, no internal loop, matching the workflow's own long-stale "single pass" comment; cron cadence (now the only polling interval) reduced from every 5 min to every 15 min in `mega_scout.yml`. (2) `run_live_pass()` only polls live games where an open position already exists — a pending ML/RUNLINE/TOTAL/PROP bet from today's scout, read via `_open_live_positions()`/`_game_has_open_position()` — and skips the run entirely (zero Odds API calls) logging `"no live positions, skipping"` when none exist anywhere tonight. (3) `market_engine._odds_request()` now captures and logs The Odds API's `x-requests-remaining`/`x-requests-used` response headers on every call via the new `odds_quota.py`, which also implements a hard guard: once cumulative usage crosses 80% of `ODDS_MONTHLY_ALLOWANCE` (default 500), `live_engine` is disabled for the rest of the calendar month (persisted in git-committed `odds_quota_state.json`, since GH Actions jobs share no local DB) and exactly one Telegram alert is sent. The guard only ever gates `live_engine` — scouts never consult it and always run regardless of quota level, since they're the actual product. (`test_odds_quota.py`, `test_live_engine_quota.py`) |
| Sync-to-Railway failures were silent | `db.push_bet_to_railway()` returned `False` with no log/alert whenever `RAILWAY_SYNC_URL`/`SYNC_SECRET` were unset — a GH Actions scout run would silently sync zero picks while reporting success, leaving the worker's settlement/CLV/Monitor/Analyst loops running on an empty database. Fixed 2026-09-09: `push_bet_to_railway()` now prints a loud warning and logs via `error_logger` (once per process, via `_running_on_railway()` detection so Railway itself — which intentionally never pushes to itself — never triggers it); `brain.py` tracks push attempts/successes per scout run (`_sync_stats`/`_reset_sync_stats`) and prints `[SYNC] pushed N/M picks to Railway` at the end of every run, alerting via Telegram if `M > 0` and `N == 0`. A new `monitor_agent.check_no_bets_on_game_day` check catches the same failure from the receiving end — alerts if worker's own DB has zero bets for today after 4pm ET on a day MLB actually played games. (`test_github_bet_sync.py`, `test_monitor_agent.py`) |
| Dead odds feed looked like an empty slate | On 2026-08-31 every game logged `[MKT] odds request failed`, the scout finished in 39s, GitHub Actions went green, and Telegram was silent — nothing distinguished "no games today" from "feed is dead." Fixed 2026-09-09: `market_engine._odds_request()` now records the actual HTTP status (401/429/etc.) via `data_health.record_ok("odds", False, detail=...)`; `brain.analyze_game()` calls the new `data_health.mark_skip_no_odds()` on its exact no-market-data skip path; `run_daily_scout()` compares that count against `_slate_attempted` (games actually handed to `analyze_game()` this window) right after the per-game loop, and sends `🚨 ODDS FEED FAILURE — X/Y games skipped for no odds. Check API quota. (HTTP nnn)` when more than half the slate was skipped for no odds. Wired off `data_health`'s existing signal rather than a parallel detection path. (`test_odds_feed_failure.py`) |
| Doubleheader Game 2 silently analyzed as Game 1 → duplicate prop picks (and Game 2 gets zero real coverage) | Initial 2026-07-29 diagnosis (from a static runlog) wrongly blamed The Odds API for listing one game twice; a live verification run the same day disproved that — `market_engine.get_mlb_events()`/`_odds_request` correctly returned two distinct events for ATL@NYM's real day-night doubleheader (different IDs, commence_time ~6h apart: 17:10 UTC vs 23:10 UTC). The **actual** root cause: `brain.py`'s `_resolve_game_pk(team_name, game_date)` queried MLB's schedule endpoint and returned the FIRST team-name match unconditionally, with no way to pick between Game 1 (gamePk 823596, AJ Smith-Shawver/Sean Manaea) and Game 2 (gamePk 823598, Chris Sale/Christian Scott) — so **both** Odds-API events resolved to Game 1's gamePk, and Game 2 got Game 1's lineups/pitchers/props analyzed a second time under its own event, byte-for-byte identical to Game 1's real picks. Game 2 itself never got any real analysis. Fixed by threading `commence_utc` through `_resolve_game_pk`, which now picks whichever schedule match's real start time is closest to it when a team-pair has multiple games that day (`test_doubleheader_game_pk.py` — includes a regression test for the exact ATL@NYM DH case, using live-verified gamePks/pitchers/times). `market_engine._dedup_events()` (dedupes same-team-pair events **only** within a 90-minute window, so it never touches a real doubleheader) was added first, based on the wrong initial theory — kept anyway as a harmless second line of defense against The Odds API ever genuinely double-listing an event, but it was not the fix for this bug. Same for the slip-assembly-level dedup guard in `_daily_bet_slip` (`(player, stat, game)`, `test_prop_dedup.py`): it's correct behavior for true duplicates, but for this specific case it was silently (and wrongly) swallowing Game 2's picks as "duplicates" of Game 1's — a symptom of the real bug, not a safety net for it. A full audit of `parlay_os.db` found zero already-graded duplicate PROP rows needing cleanup. **Lesson**: a static log can suggest the wrong root cause even when the fix based on it doesn't do harm — a live re-run against the same trigger conditions (here, today's actual doubleheader) is what surfaced the real bug. |
