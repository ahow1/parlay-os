"""
PARLAY OS - MEGA SCOUT v5
Short prompt, robust parsing.
"""
...
Return ONLY valid JSON. No text outside the JSON. No apostrophes in strings."""
...
def run_mega_scout():
    ...
    for name, fn in [
        ...
        ("ascii_only",     lambda x: json.loads(x.encode('ascii', errors='ignore').decode('ascii'))),
    ]:
        try:
            data = fn(json_str)
            print(f"Parsed with: {name} — {len(data.get('games',[]))} games")
            return data
        except Exception as je:
            print(f"{name} failed: {str(je)[:80]}")
...
def format_message(data, clv_stats=None):
    ...
    if legs:
        ...
        for l in legs: m1.append(f"  {l}")

    if data.get("session_note"): m1.append(f"\n{data['session_note']}")
    messages.append("\n".join(m1))
...
def main():
    ...
    try:
        ...
        for pick in data.get("locks",[]) + data.get("coinflips",[]):
            ...
            entries.append({"date":today,"bet":pick,"type":"F5" if "F5" in pick else "NRFI" if "NRFI" in pick else "ML","bet_odds":odds,"closing_odds":None,"result":None,"clv_pct":None})

        if entries:
            clv_log.extend(entries)
            save_clv_log(clv_log)

        with open("last_scout.json","w") as f: json.dump(data, f, indent=2)
        ...
        print(f"FATAL: {e}")
        try: send_telegram(f"PARLAY OS Failed\n{str(e)}\n{datetime.now(ET).strftime('%I:%M %p ET')}")
        except: pass
        raise
